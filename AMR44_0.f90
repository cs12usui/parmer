...
!***********************************************************************
        ...
        integer(kind=4),parameter :: Nstep = 2000        ! of calculation step (end)
        ...
        integer(kind=4),parameter :: DataIntvl2= 100      ! Interval for data output2
        ...
        real(kind=8)    :: crtr0(0:LvMax), crtr1(0:LvMax), crtrYZ0(0:LvMax), crtrYZ1(0:LvMax) ! criterion
...
!Note from Tatsuki: NXB, NYB, and NZB have to be 2^n in order to run properly (at least for now)
        integer(kind=4), parameter:: NXR = 16, NYR = 2, NZR = 2 ! NXR*NYR*NZR Must be equal to the total process number
        integer(kind=4), parameter:: NX = 8, NY=32, NZ=32        ! NX, NY, and NZ have to be equal to or larger than 8.
        integer(kind=4), parameter:: npart_per_cell=256         ! number of particle per cell
...
!yagi added
        ...
        integer(kind=4), parameter::Joutput       = 1 !charge
        ...
        integer(kind=4), parameter::eleJoutput    = 1 !only electron
        integer(kind=4), parameter::ionJoutput    = 1 !only ion
...
!===== Parameters for Mesh =====
        !for AMR parameters
        real(kind=8),parameter:: refineRatio(-1:10) = (/ 1.d0, 1.0d0, 0.7d0, 0.0d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0, 0.d0 /)
        !                                                Lv-1,   Lv0,   Lv1,  Lv2,  Lv3,  Lv4,  Lv5,  Lv6,  Lv7,  Lv8,  Lv9, Lv10
        integer(kind=4)::LevelSize(-1:10),GLevelSize(-1:10)
        integer(kind=4),parameter::BMeshSize=Nall_ini/8*40
        ...
        subroutine set_AMR_Criterion
          ...
          if(AMRcondition==1)then
            ...
            if(LvMax.eq.2) then
                ...
                crtr1(0)=dble(npart_per_cell)*0.2
                crtr0(1)=999.d0 ! 0.40d0
                crtr1(1)=dble(npart_per_cell)*1.0
...
!                    else
                     !   crtr(iLv)=crtr0(iLv)
!                    end if
                    if((Fp1>=crtr0(iLv)).and.(Fp1<=crtr1(iLv)))then
                    if((Fp2>=crtrYZ0(iLv)).and.(Fp2<=crtrYZ1(iLv)))then
                    if((Fp3>=crtrYZ0(iLv)).and.(Fp3<=crtrYZ1(iLv)))then
                        p0%iFLG(3)=1
                    end if
                    end if
                    end if
...
!                        else
                            !crtr(iLv)=crtr0(iLv)
!                        end if
                        if((Fq1>=crtr0(iLv)).and.(Fq1<=crtr1(iLv)))then
                        if((Fq2>=crtrYZ0(iLv)).and.(Fq2<=crtrYZ1(iLv)))then
                        if((Fq3>=crtrYZ0(iLv)).and.(Fq3<=crtrYZ1(iLv)))then
                            p0%iFLG(3)=1
                        end if
                        end if
                        end if
  ...
  if (p0%iFLG(1) .ge. 4) then
    ...
    do while (dble(num) .lt. dble(npart_per_cell)*ratio*(sLv**3.0d0))
     ...
     ! -- strage to the list structure for particles --
      PrtList => p0%ptcl%prtnxt
      Pesh(j,iLv)=prtcl(R,particle,p0%octN,j,PrtList,1)
...
subroutine output_Bfield_point
use oct_set 
use param
use const 
use message_passing_interface
...
type(oct),pointer :: p0
	if(istep == 1) then
	open(95,file='./output/Bfield_point.dat',status='replace') 
	call get_meshID(116,252,252)
	else
	open(95,file='./output/Bfield_point.dat',status='old',position='append')
	end if
	p0 => Mesh(point)
	do i=0,LvMax
		if(p0%iFLG(1)>3) then
	...
	oF(3)=p0%F(6)+FIMF(6)
	if(rank==0) then
		write(95,'(i6,3f14.7)')istep,oF(1:3)
	end if
	close(95)
end subroutine output_Bfield_point



subroutine get_meshID(x,y,z)
use oct_set 
use param
use const 
use init_mesh_size
...
type(oct),pointer :: p0
    do index=MinID(1,0),MaxID(1,0)
        p0=>Mesh(index)
	    if(p0%iPos(1)==x .and. p0%iPos(2)==y .and. p0%iPos(3)==z) then
			point = index
				print *,"istep=",istep,"point=",point
		end if
	end do
end subroutine get_meshID
...
subroutine makeInjectionOctTableArray(size)
    ...
    do index=MinID(1,0),MaxID(1,0)
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==212.and.p0%iPos(3)==244)then
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==212.and.p0%iPos(3)==252)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==228)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==236)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==244)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==252)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==220)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==228)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==236)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==244)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==252)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==220)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==228)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==236)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==244)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==252)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==244.and.p0%iPos(3)==212)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==244.and.p0%iPos(3)==220)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==244.and.p0%iPos(3)==228)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==244.and.p0%iPos(3)==236)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==244.and.p0%iPos(3)==244)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==244.and.p0%iPos(3)==252)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==252.and.p0%iPos(3)==212)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==252.and.p0%iPos(3)==220)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==252.and.p0%iPos(3)==228)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==252.and.p0%iPos(3)==236)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==252.and.p0%iPos(3)==244)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==252.and.p0%iPos(3)==252)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==212.and.p0%iPos(3)==260)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==212.and.p0%iPos(3)==268)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==260)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==268)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==276)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==220.and.p0%iPos(3)==284)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==260)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==268)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==276)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==284)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==228.and.p0%iPos(3)==292)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==260)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==268)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==276)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==284)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)==236.and.p0%iPos(3)==292)then
            InjectionTableArray(i)=injectionStructure(p1%octN, 2, 0d0, 1.5707963267949d0, 1d0)
            p1%rPos = p0%rPos
            p1%octNb2 => p0%octNb2
            i = i + 1
        ...
        if(p0%iPos(1)==68.and.p0%iPos(2)